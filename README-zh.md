...
这个项目的产品定位不是“又一个网页清洗器”，而是一个尽量尊重原网页结构的双语阅读工具：插件不替换原文，不把 API Key 暴露给 content script，也不会自动传输网页文本。最初 MVP 聚焦全文翻译主链路；当前版本已经在同一套 provider 与任务调度框架上扩展了划词翻译、YouTube 字幕翻译和全文总结。
...
- 划词翻译：通过右键菜单翻译选中文本，并在页面内弹窗展示结果。
- 全文总结：通过 OpenAI-compatible provider 对可读页面内容生成摘要。
- YouTube 字幕翻译：翻译支持的 YouTube 字幕轨道，并在视频上方展示译文。
